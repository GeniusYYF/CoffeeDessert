<template>
	<el-menu :default-active="activeIndex" class="header" mode="horizontal" router @select="handleSelect">
...
<style lang="scss" scoped>
	.header {
		background: radial-gradient(rgba(248, 193, 62, .8), transparent);
		...
		>.el-submenu .el-submenu__title:hover {
			background-color: rgba($color: rgb(253, 244, 196), $alpha: .3);
		}
		...
		.header-title {
			...
			h1 {
				...
				font-size: 40px;
				font-family: 楷体;
				color: rgba($color: #fc7e00, $alpha: .8);
				text-shadow: 3px 3px 5px rgba($color: #d1a023, $alpha: 1.0);
			}
		}
